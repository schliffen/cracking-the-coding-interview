...
BinaryTree<int>* getRandomNodeWithFreeSpace(BinaryTree<int>* pRoot, int randLeafDepth, int currentDepth)
{
    assert(pRoot);
    ...
    case 1:
        ...
        {
            BinaryTree<int>* pLeaf = getRandomNodeWithFreeSpace(pRoot->left ? pRoot->left : pRoot->right, randLeafDepth, currentDepth + 1);
            return pLeaf;
        }
    // node is full, we have have no space so let's keep on exploring
    case 2:
            int direction = rand() % 2;
            BinaryTree<int>* pLeaf = getRandomNodeWithFreeSpace(direction ? pRoot->left : pRoot->right, randLeafDepth, currentDepth + 1);
            return pLeaf;
    }
    assert(false); // should not reach
    return 0; // suppress compiler warning
...
void buildRandomBinaryTree(BinaryTree<int>* pRoot, int limit, idFunci idFunc)
{
    ...
    {
        int randLeafDepth = rand() % maxDepth*2;
        BinaryTree<int>* pLeaf = getRandomNodeWithFreeSpace(pRoot, randLeafDepth);
        ...
        case 0:
            ...
            {
            case 0:
                pLeaf->left = new BinaryTree<int>((*idFunc)());
                count += 1;
                if (count < limit)
                {
                    pLeaf->right = new BinaryTree<int>((*idFunc)());
                    count += 1;
                }
                break;
            case 1:
